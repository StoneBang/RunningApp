...
public class HomeAdapter extends BasicAdapter<AppInfo>{
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		//增加放大的弹性动画
		ViewHelper.setScaleX(convertView, 0.6f);
		ViewHelper.setScaleY(convertView, 0.6f);
		ViewPropertyAnimator.animate(convertView).scaleX(1f).setDuration(350).setInterpolator(new OvershootInterpolator()).start();
		ViewPropertyAnimator.animate(convertView).scaleY(1f).setDuration(350).setInterpolator(new OvershootInterpolator()).start();
